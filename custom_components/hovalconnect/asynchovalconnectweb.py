...
class HovalconnectWebSession(object):
    ...
    async def _submit_form(self, form_details):
        ...
        if form_details["method"] == "post":
            response = await self.session.post(url, data=form_details["data"])
        elif form_details["method"] == "get":
            response = await self.session.get(url, params=form_details["data"])
        self.last_url = url
        return response
    ...
    async def login(self):
        ...
        _LOGGER.info("Trying to log in to connect to %s", URL_START)

        # request 1
        response = await self.session.get(URL_START)
        ...
        while self.last_url == "" or not self.last_url == URL_START:
            response = await self._submit_form(form_details)
            print("getting url " + response.url)
    ...
    async def get_latest_events(self):
        ...
        response = await self.session.get(URL_LATEST_PLANT_EVENTS)
        return response

    async def set_plant(self, plant_id: str):
        """ "set plant by id"""
        _LOGGER.info("Setting plant to %s", plant_id)

        self.plant_id = plant_id
        response = await self._get_json(URL_SETTINGS_PLANT + self.plant_id)
    ...
    async def get_live_state(self):
        """get live state"""
        _LOGGER.debug("Getting live state for plant %s", self.plant_id)
        response = await self._get_json(URL_SNAPSHOTS_LIVE + self.plant_id, True, True)
        return response
